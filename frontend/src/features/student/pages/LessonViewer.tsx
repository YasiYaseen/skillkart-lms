...
function LessonViewer() {
    ...
    useEffect(() => {
        ...
        const fetchProgress = async () => {
            try {
                ...
                setIsCourseCompleted(Boolean(p.isCompleted));
                setHasNewLessons(Boolean(p.hasNewLessons));
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-slate-50 dark:bg-slate-900">
            ...
            {/* Status Banner when enrolled student completed course but new lessons were added */}
            {isCourseCompleted && completedLessonIds.length < lessons.length && (
                <div className="bg-blue-50 dark:bg-blue-950/50 border-b border-blue-200 dark:border-blue-800/60 px-4 py-2.5 text-xs text-blue-800 dark:text-blue-200 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-2">
                        <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 dark:bg-emerald-900/60 px-2 py-0.5 text-[10px] font-semibold text-emerald-800 dark:text-emerald-200">
                            <CheckIcon className="w-3 h-3" />
                            Course Completed
                        </span>
                        <span>
                            You previously completed this course and earned your certificate. The instructor has added {lessons.length - completedLessonIds.length} new lesson{lessons.length - completedLessonIds.length > 1 ? 's' : ''} since graduation. Your certificate remains active and valid.
                        </span>
                    </div>
                    <Link to="/certificates" className="text-[11px] font-semibold text-blue-600 dark:text-blue-400 hover:underline shrink-0 ml-3">
                        View Certificate ↗
                    </Link>
                </div>
            )}
                    ...
                    <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5 mb-1.5 overflow-hidden">
                        <div
                            className={`h-1.5 rounded-full transition-all duration-300 ${
                                !hasAccess ? 'bg-amber-400/40' : isCourseCompleted ? 'bg-emerald-600' : 'bg-blue-600'
                            }`}
                            style={{ width: `${hasAccess ? progressPercentage : 0}%` }}
                        ></div>
                    </div>
                    {hasAccess && isCourseCompleted && completedLessonIds.length < lessons.length ? (
                        <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium">
                            Certificate earned • {lessons.length - completedLessonIds.length} new lesson{lessons.length - completedLessonIds.length > 1 ? 's' : ''} available
                        </p>
