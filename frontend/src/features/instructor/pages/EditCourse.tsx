...
function EditCourse() {
    ...
    const handleResubmitForModeration = async () => {
        ...
        try {
            const res = await api.patch<{ message?: string; course?: RawCourseData }>(`/courses/${courseId}/publish`);
            const updatedCourse = res.data?.course;
            setCourseStatus(updatedCourse?.status || 'published');
            setIsApproved(updatedCourse?.isApproved);
            setRejectionReason(updatedCourse?.rejectionReason);
            toast.success(res.data?.message || 'Course resubmitted for moderation successfully!');
        } catch (err: unknown) {
            toast.error(getErrorMessage(err, 'Failed to resubmit course for moderation'));
        } finally {
    ...
    const handlePublishOrSubmit = async () => {
        ...
        try {
            ...
            toast.error(getErrorMessage(err, 'Failed to submit course'));
        } finally {
    ...
    const handleUnpublishToDraft = async () => {
        ...
        try {
            ...
            const updatedCourse = res.data?.course;
            setCourseStatus(updatedCourse?.status || 'draft');
            setIsApproved(updatedCourse?.isApproved);
            setRejectionReason(updatedCourse?.rejectionReason);
            toast.success(res.data?.message || 'Course moved to draft');
        } catch (err: unknown) {
            toast.error(getErrorMessage(err, 'Failed to unpublish course'));
        } finally {
    ...
    const handleUnarchiveCourse = async () => {
        ...
        setUnarchiving(true);
        try {
            const res = await api.patch<{ message?: string; course?: RawCourseData }>(`/courses/${courseId}/unarchive`);
            const updatedCourse = res.data?.course;
            setCourseStatus(updatedCourse?.status || 'draft');
            setIsApproved(updatedCourse?.isApproved);
            setRejectionReason(updatedCourse?.rejectionReason);
            toast.success(res.data?.message || 'Course unarchived and restored');
        } catch (err: unknown) {
    ...
    const handleArchiveCourse = async () => {
        ...
        setArchiving(true);
        try {
            const res = await api.patch<{ message?: string; course?: RawCourseData }>(`/courses/${courseId}/archive`);
            const updatedCourse = res.data?.course;
            setCourseStatus(updatedCourse?.status || 'archived');
            setIsApproved(updatedCourse?.isApproved);
            setRejectionReason(updatedCourse?.rejectionReason);
            toast.success(res.data?.message || 'Course archived successfully');
        } catch (err: unknown) {
            toast.error(getErrorMessage(err, 'Failed to archive course'));
        } finally {
            setArchiving(false);
        }
    };
        ...
        <div className="max-w-4xl space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2.5 self-start sm:self-auto">
                    ...
                    {/* Archived course: Restore / Unarchive */}
                    {isActive !== false && courseStatus === 'archived' && (
                        <button
                            type="button"
                            onClick={handleUnarchiveCourse}
                            disabled={unarchiving}
                            className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-xl transition-colors shadow-2xs cursor-pointer"
                            title="Unarchive and restore this course to draft or published status"
                        >
                            {unarchiving ? (
                                <>
                    ...
                    {isActive !== false && isApproved === false && (
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-xl transition-colors shadow-2xs cursor-pointer"
                            title="Resubmit your revised course for administrator review"
                        >
                            {resubmitting ? (
                                <>
                                    <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                                ...
                                <>
                                    ...
                                    <span>Resubmit for Moderation</span>
                                </>
                            )}
                        </button>
                    )}
                    ...
                    {isActive !== false && courseStatus === 'draft' && isApproved !== false && (
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 rounded-xl transition-colors shadow-2xs cursor-pointer"
                            title="Submit course for publication and moderation review"
                        >
                            {submittingPublish ? (
                                <>
                                    <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                    ...
                    {isActive !== false && courseStatus === 'published' && (
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-slate-700 dark:text-slate-200 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 disabled:opacity-50 rounded-xl transition-colors border border-slate-300 dark:border-slate-700 shadow-2xs cursor-pointer"
                            title="Unpublish course back to draft for revisions. It will require re-moderation before going live."
                        >
                            {unpublishing ? (
                                <>
                    ...
                    {/* Active course (draft or published): Archive course */}
                    {isActive !== false && courseStatus !== 'archived' && (
                        <button
                            type="button"
                            onClick={handleArchiveCourse}
                            disabled={archiving}
                            className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-amber-700 dark:text-amber-300 bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/40 dark:hover:bg-amber-900/50 disabled:opacity-50 rounded-xl transition-colors border border-amber-200 dark:border-amber-800 shadow-2xs cursor-pointer"
                    ...
                    {courseId && (
                        <a
                            ...
                            className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/60 dark:text-blue-300 rounded-xl transition-colors border border-blue-200 dark:border-blue-800 shadow-2xs"
                        >
            ...
            {isActive !== false && courseStatus === 'archived' && (
                <div className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60 text-gray-800 dark:text-gray-200 flex items-start justify-between gap-3 shadow-xs">
                    <div className="flex items-start gap-3">
                        <div className="p-2 rounded-lg bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 shrink-0">
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5m8.25 3v6.75m0 0l-3-3m3 3l3-3M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                            </svg>
                        </div>
                        <div className="text-xs">
                            <p className="font-bold text-sm text-gray-900 dark:text-gray-100">Course is Archived</p>
                            <p className="mt-0.5 text-gray-600 dark:text-gray-300">
                                This course is currently archived and hidden from public course listings. You can restore it anytime to make it available again.
                            </p>
                        </div>
                    </div>
                    <button
                        ...
                        onClick={handleUnarchiveCourse}
                        disabled={unarchiving}
                        className="shrink-0 px-3 py-1.5 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-xs transition-colors cursor-pointer"
                    >
                        {unarchiving ? 'Restoring...' : 'Restore Course'}
                    </button>
                </div>
            )}
            ...
            {isActive !== false && isApproved === false && (
                <div className="p-4 rounded-xl border border-rose-200 dark:border-rose-800 bg-rose-50 dark:bg-rose-950/40 text-rose-900 dark:text-rose-200 flex flex-col sm:flex-row items-start justify-between gap-4 shadow-xs">
                    ...
                    <button
                        ...
                        onClick={handleResubmitForModeration}
                        disabled={resubmitting}
                        className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-rose-600 hover:bg-rose-700 disabled:opacity-50 rounded-xl transition-colors shadow-2xs shrink-0 self-start sm:self-center cursor-pointer"
                        title="Resubmit your revised course for administrator review"
                    >
                        {resubmitting ? (
                            <>
